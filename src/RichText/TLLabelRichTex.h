...
protected:
    ...
    float createChatSprite( chatTexInfo* pCTInfo, float fScale, float fOriginOffsetX, bool bAutoWrap, const CCSize& kContentSize );
public:
	void adjustLayout();
...
public:
    ...
    void setEnableShadow( bool bEnable ) { m_bEnableShadow = bEnable; }
    bool getEnableShadow() const { return m_bEnableShadow; }

    void setShadowBlur( float fShadowBlur ) { m_fShadowBlur = fShadowBlur; }
    float getShadowBlur() const { return m_fShadowBlur; }

    void setShadowColor( const GLubyte r, const GLubyte g, const GLubyte b, const GLubyte a ) { m_kShadowColor = ccc4(r,g,b,a); }
    void setShadowColorR( const GLubyte r ) { m_kShadowColor.r = r; }
    void setShadowColorG( const GLubyte g ) { m_kShadowColor.g = g; }
    void setShadowColorB( const GLubyte b ) { m_kShadowColor.b = b; }
    void setShadowColorA( const GLubyte a ) { m_kShadowColor.a = a; }
    ccColor4B getShadowColor() const { return m_kShadowColor; }

    void setEnableStroke( bool bEnable ) { m_bEnableStroke = bEnable; }
    bool getEnableStroke() const { return m_bEnableStroke; }
...
protected:
    ...
    float m_fShadowBlur;
	ccColor4B m_kShadowColor;

    // enable stroke
    bool m_bEnableStroke;
    float m_fStrokeWidth;

    std::set<CCNode*> m_setShadowStrokeNodes;
};
