...
void TLLabelRichTex::appendText( const char* szText )
{
    ...
    while( strTemp.length() > 0 )
    {
        ...
        if( m_eRichStringFlag == TL_RICH_STRING_FLAG_ONE_LINE_DOTDOTDOT )
        {
            if( m_kCurrentLineInfo.fLineRealWidth + m_fFontSize * 2 > kContentSize.width )
            {
                for( int j=0; j<3; ++j )
                {
                    ...
                    createChatSprite( pCTInfo, fScale, m_fFontSize * j / 3, false, kContentSize );
                }
                break;
            }
        }
        ...
        if( pTemp[0] == '[' )   // 转义 
        {
            unsigned int nPos = strTemp.find( ']' );
            if( nPos != std::string::npos )
            {
                std::string strSub = strTemp.substr( 1, nPos - 1 );

                unsigned int nPos1 = strSub.find( ':' );
        ...
        float fTempWidth = createChatSprite( pCTInfo, fScale, 0.0f, true, kContentSize );
...
void TLLabelRichTex::adjustLayout()
{
	...
	for( int i=0; i < (int)m_vecLine.size(); ++i )
	{
		...
		for( ; iter != iter_end; ++iter )
		{
            ...
            std::set<CCNode*>::iterator iterSS = m_setShadowStrokeNodes.find( pkNode );
            if( iterSS != m_setShadowStrokeNodes.end() )
            {
                pkNode->setPosition( fLastX + iter->fOffsetX, fLastY + iter->fOffsetY );
            ...
            fLastX = fOriginX + offset_x;
            fLastY = real_offset_y + fixY;
			pkNode->setPosition( fLastX, fLastY );
...
float TLLabelRichTex::createChatSprite( chatTexInfo* pCTInfo, float fScale, float fOriginOffsetX, bool bAutoWrap, const CCSize& kContentSize )
{
    float fRetWidth = 0.0f;
    ...
    if( bAutoWrap )
    {
        fRetWidth = pCTInfo->rcUV.size.width * fScale;
        if( m_eRichStringFlag == TL_RICH_STRING_FLAG_AUTO_WRAP && kContentSize.width > 0.0f && m_kCurrentLineInfo.fLineRealWidth + fRetWidth > kContentSize.width )
            appendLine( true );
    }

    float fAdditionalWidth = 0.0f;
    std::list<NodeInfo> listShadowStrokeNodes;
    // 阴影
    if( m_bEnableShadow )
    {
        CCSprite* pkShadowSprite = CCSprite::createWithTexture( m_pBatchNode->getTexture(), pCTInfo->rcUV );
        pkShadowSprite->setAnchorPoint( CCPointZero );
        pkShadowSprite->setColor( ccc3( m_kShadowColor.r, m_kShadowColor.g, m_kShadowColor.b ) );
        pkShadowSprite->setOpacity( m_kShadowColor.a );
        pkShadowSprite->setScale( fScale );
        //pkShadowSprite->setTexture2Coords( ul, vt, ur, vb );

        m_pBatchNode->addChild( pkShadowSprite, 0, m_nElementCount++ );

        NodeInfo ni;
        ni.eType = Font;
        ni.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
        ni.pkNode = pkShadowSprite;
        ni.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale, fCommonLineHeight * fScale );
        //ni.fOffsetX = m_fShadowBlur;
        ni.fOffsetX = 0.0f;
        ni.fOffsetY = -m_fShadowBlur;
        listShadowStrokeNodes.push_back( ni );

        m_setShadowStrokeNodes.insert( pkShadowSprite );

        //fAdditionalWidth = m_fShadowBlur * 2;
    }
    ...
    {
        // 上
        CCSprite* pkStrokeSpriteUp = CCSprite::createWithTexture( m_pBatchNode->getTexture(), pCTInfo->rcUV );
        pkStrokeSpriteUp->setAnchorPoint( CCPointZero );
        pkStrokeSpriteUp->setColor( ccc3( m_kShadowColor.r, m_kShadowColor.g, m_kShadowColor.b ) );
        pkStrokeSpriteUp->setOpacity( m_kShadowColor.a );
        pkStrokeSpriteUp->setScale( fScale );
        //pkStrokeSpriteUp->setTexture2Coords( ul, vt, ur, vb );

        m_pBatchNode->addChild( pkStrokeSpriteUp, 0, m_nElementCount++ );

        NodeInfo niUp;
        niUp.eType = Font;
        niUp.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
        niUp.pkNode = pkStrokeSpriteUp;
        niUp.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale, fCommonLineHeight * fScale );
        niUp.fOffsetX = 0.0f;
        niUp.fOffsetY = m_fStrokeWidth;
        listShadowStrokeNodes.push_back( niUp );

        m_setShadowStrokeNodes.insert( pkStrokeSpriteUp );

        // 下
        CCSprite* pkStrokeSpriteDown = CCSprite::createWithTexture( m_pBatchNode->getTexture(), pCTInfo->rcUV );
        pkStrokeSpriteDown->setAnchorPoint( CCPointZero );
        pkStrokeSpriteDown->setColor( ccc3( m_kShadowColor.r, m_kShadowColor.g, m_kShadowColor.b ) );
        pkStrokeSpriteDown->setOpacity( m_kShadowColor.a );
        pkStrokeSpriteDown->setScale( fScale );
        //pkStrokeSpriteDown->setTexture2Coords( ul, vt, ur, vb );

        m_pBatchNode->addChild( pkStrokeSpriteDown, 0, m_nElementCount++ );

        NodeInfo niDown;
        niDown.eType = Font;
        niDown.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
        niDown.pkNode = pkStrokeSpriteDown;
        niDown.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale, fCommonLineHeight * fScale );
        niDown.fOffsetX = 0.0f;
        niDown.fOffsetY = -m_fStrokeWidth;
        listShadowStrokeNodes.push_back( niDown );

        m_setShadowStrokeNodes.insert( pkStrokeSpriteDown );

        // 左
        CCSprite* pkStrokeSpriteLeft = CCSprite::createWithTexture( m_pBatchNode->getTexture(), pCTInfo->rcUV );
        pkStrokeSpriteLeft->setAnchorPoint( CCPointZero );
        pkStrokeSpriteLeft->setColor( ccc3( m_kShadowColor.r, m_kShadowColor.g, m_kShadowColor.b ) );
        pkStrokeSpriteLeft->setOpacity( m_kShadowColor.a );
        pkStrokeSpriteLeft->setScale( fScale );
        //pkStrokeSpriteLeft->setTexture2Coords( ul, vt, ur, vb );

        m_pBatchNode->addChild( pkStrokeSpriteLeft, 0, m_nElementCount++ );

        NodeInfo niLeft;
        niLeft.eType = Font;
        niLeft.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
        niLeft.pkNode = pkStrokeSpriteLeft;
        niLeft.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale, fCommonLineHeight * fScale );
        niLeft.fOffsetX = -m_fStrokeWidth;
        niLeft.fOffsetY = 0.0f;
        listShadowStrokeNodes.push_back( niLeft );

        m_setShadowStrokeNodes.insert( pkStrokeSpriteLeft );

        // 右
        CCSprite* pkStrokeSpriteRight = CCSprite::createWithTexture( m_pBatchNode->getTexture(), pCTInfo->rcUV );
        pkStrokeSpriteRight->setAnchorPoint( CCPointZero );
        pkStrokeSpriteRight->setColor( ccc3( m_kShadowColor.r, m_kShadowColor.g, m_kShadowColor.b ) );
        pkStrokeSpriteRight->setOpacity( m_kShadowColor.a );
        pkStrokeSpriteRight->setScale( fScale );
        //pkStrokeSpriteRight->setTexture2Coords( ul, vt, ur, vb );

        m_pBatchNode->addChild( pkStrokeSpriteRight, 0, m_nElementCount++ );

        NodeInfo niRight;
        niRight.eType = Font;
        niRight.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
        niRight.pkNode = pkStrokeSpriteRight;
        niRight.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale, fCommonLineHeight * fScale );
        niRight.fOffsetX = m_fStrokeWidth;
        niRight.fOffsetY = 0.0f;
        listShadowStrokeNodes.push_back( niRight );

        m_setShadowStrokeNodes.insert( pkStrokeSpriteRight );

        //if( fAdditionalWidth < m_fStrokeWidth * 2 )
        //    fAdditionalWidth = m_fStrokeWidth * 2;
    }
    ...
    ni.fOriginX = m_kCurrentLineInfo.fLineRealWidth + fOriginOffsetX;
    ni.pkNode = pkSprite;
    ni.currentSize = CCSizeMake( pCTInfo->rcUV.size.width * fScale + fAdditionalWidth, fCommonLineHeight * fScale );
    m_kCurrentLineInfo.listLineNode.push_back( ni );

    std::list<NodeInfo>::iterator iter = listShadowStrokeNodes.begin();
    std::list<NodeInfo>::iterator iter_end = listShadowStrokeNodes.end();
    for( ; iter != iter_end; ++iter )
        m_kCurrentLineInfo.listLineNode.push_back( *iter );
